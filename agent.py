...
class MADDPG_Agents():
    ...
    def learn(self):
        ...
        # for agent_idx in range(self.n_agents):            
        #     obs_batch = torch.FloatTensor(all_obs_batch[:, agent_idx, :]).to(cf.device)
        #     next_obs_batch = torch.FloatTensor(all_next_obs_batch[:, agent_idx, :]).to(cf.device)
        ...
        # joint_mu = torch.FloatTensor(joint_mu).reshape((self.batch_size, -1)).to(cf.device)
        # joint_target_mu_batch = torch.FloatTensor(joint_target_mu_batch).reshape((self.batch_size, -1)).to(cf.device)
